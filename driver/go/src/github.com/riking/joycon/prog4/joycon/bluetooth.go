...
func (jc *joyconBluetooth) EnableGyro(status bool) {
	...
	fmt.Printf("gyro successfully enabled? : %t\n", status)
...
func (jc *joyconBluetooth) ReadInto(out *jcpc.CombinedState, includeGyro bool) {
	...
	//jc.gyro[0][0] / 2					// pitch axis
	//jc.gyro[1][0] / 2					// pitch axis (inverted)
	//jc.gyro[0][1] / 2					// roll axis
	//jc.gyro[1][1] / 2					// roll axis (again)
	//jc.gyro[0][2] / 2					// roll axis (again but zeroes at minus value)
	//jc.gyro[1][2] / 2					// roll axis (again but zeroes at minus value)
	//jc.gyro[0][3] / 2					// roll rate of change
	//jc.gyro[1][3] / 2					// roll rate of change (again)
	//jc.gyro[0][4] / 2					// pitch rate of change
	//jc.gyro[1][4] / 2					// pitch rate of change (again)
	//jc.gyro[0][5] / 2					// yaw rate of change
	//jc.gyro[1][5] / 2					// yaw rate of change (again)

	out.Buttons = out.Buttons.Remove(jc.side).Union(jc.buttons)
	if jc.side.IsLeft() {
		out.AdjSticks[0] = jc.calib[0].Adjust(jc.raw_stick[0])
		out.AdjSticks[0][0] = jc.gyro[0][0] / 2						// pitch axis
		out.AdjSticks[0][1] = jc.gyro[0][1] / 2						// roll axis
	}
	if jc.side.IsRight() {
...
func (jc *joyconBluetooth) fillGyroData(packet []byte) {
    ...
    // enable this! gyroPrint(jc.gyro[0])
